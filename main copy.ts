...
let malaquias = new Cliente('malaquias', "4613213849", 'malaquias@gmail.com');
...
try {
    malaquias.validarCPF(malaquias.cpf);  // Vai verificar se a quantidade de digitos bate com 11, se não, vai mostrar erro
} catch (error) {
    if (error instanceof PessoaNaoEncontradaError) {
        console.error(error.message);
    } else {
        console.error("Ocorreu um erro inesperado.");
    }
